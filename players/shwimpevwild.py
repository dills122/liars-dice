...
class Shwimpevwild:
    ...
    def _20(self, ctx: GameContext, quantity: int, face: int, total_dice: int, wilds: bool) -> float:
    ...
    def _24(self, ctx: GameContext, hand: list[int], total_dice: int) -> list[tuple[float, int, int]]:
        candidates = []
        unseen = total_dice - len(hand)
        factor = self._25(ctx)
    ...
    def _2a(self, ctx: GameContext, hand: list[int], quantity: int, face: int, total_dice: int) -> float:
    ...
    def algo(self, ctx: GameContext) -> Bet | None:
        ...
        current_probability = self._27(hand, ctx.prior_bet.quantity, ctx.prior_bet.face, ctx.total_dice, self._32)
        call_ev = self._1e(ctx, ctx.prior_bet, current_probability)
        scored = [(self._2a(ctx, hand, quantity, face, ctx.total_dice), quantity, face) for quantity, face in self._29(ctx.prior_bet, hand)]
